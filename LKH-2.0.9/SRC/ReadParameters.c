 ...
 * The other types, except POPMUSIC, can only be used for instances given
 * by coordinates.
...
void ReadParameters()
{
    ...
    while ((Line = ReadLine(ParameterFile))) {
        ...
        } else if (!strcmp(Keyword, "CANDIDATE_OUTPUT_FILE")) {
            if (!(CandidateOutputFileName = GetFileName(0)))
                eprintf("CANDIDATE_OUTPUT_FILE: string expected");
